		...
		return this
				.each(function() {
					...
					function startDrag(e) {
						...
						_ghost.addClass(opts.ghostClass).css('position',
								'absolute').css('z-index', '250').css(
								"-webkit-user-select", "none").width(C.width())
								.height(C.height()).css(opts.moving,
										splitter._initPos[opts.moving]);
						...
						$(document).bind("mousemove", performDrag).bind(
								"mouseup", endDrag);
					}
					...
					function endDrag(e) {
						...
						_ghost.remove();
						_ghost = null;
						mychilds.css("-webkit-user-select", "text");// let
																	// Safari
																	// select
																	// text
																	// again
					...
					function splitTo(perc, reversedorder, fast) {
						...
						_ismovingNow = true;
						if (splitPos && splitPos > 10 && splitPos < 90)// do
																		// not
																		// save
																		// accidental
																		// events
						...
						var barsize = C[opts.sizing]()
								+ (2 * parseInt(C.css('border-' + opts.moving
										+ '-width')));// + border. cehap&dirty
						var splitsize = splitter[opts.sizing]();
						if (opts.closeableto != perc) {
							var percpx = Math.max(parseInt((splitsize / 100)
									* perc), opts.minAsize);
							if (opts.maxAsize)
						...
						// A.attr('title','- '+sizeA); B.attr('title','-
						// '+sizeB);
						if (fast) {
